...
const MiningGame: React.FC = () => {
  ...
  const updateGameState = useCallback(() => {
    ...
    if (timeSinceLastIncrease >= 30000) {
      ...
      setPlayerSizeMultiplier(prev => Math.min(prev + 0.1, 2)) // Increase player size here
    ...
    setGameObjects((prevObjects) => {
      const newObjects = prevObjects
        .map((obj) => ({ ...obj, y: obj.y + 5 * speedMultiplier }))
  ...
  }, [playerPosition, createGameObject, gameOver, speedMultiplier, playerSizeMultiplier])
  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    ...
    const newPosition = e.clientX - gameRect.left - playerWidth / 2
    setPlayerPosition(Math.max(0, Math.min(newPosition, gameWidth - playerWidth)))
  }
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-r from-blue-500 to-purple-500">
      <div className="bg-white rounded-lg shadow-lg p-6 mb-4">
        <div className="text-2xl font-bold text-center text-gray-800">Score: {score}</div>
        <div className="flex justify-center items-center mt-2">
          <div className="text-lg text-gray-800">Health:</div>
          <div className="flex ml-2">
            {renderHearts()}
          </div>
        </div>
        <div className="text-sm text-gray-600 mt-2">Speed: x{speedMultiplier.toFixed(1)}</div>
        <div className="text-sm text-gray-600">Player Size: x{playerSizeMultiplier.toFixed(1)}</div>
      </div>
      <div className="relative bg-gray-200 rounded-lg overflow-hidden" style={{ width: gameWidth, height: gameHeight }} onMouseMove={handleMouseMove}>
        {gameObjects.map((obj) => (
          obj.type === 'reward' ? (
            <img
              key={obj.id}
              ...
              style={{ left: obj.x, top: obj.y, width: objectSize, height: objectSize }}
            />
          ) : (
            <img
              key={obj.id}
              ...
              style={{ left: obj.x, top: obj.y, width: 60, height: 60 }}
            />
          )
        ))}
        <img
        ...
        {countdown !== null && (
          <div className="absolute inset-0 flex items-start justify-center">
            <div className="bg-amber-500 text-white text-6xl font-bold p-4 top-4 flex items-center justify-center">
              <p className='text-sm'>Speed multiplier will start at {countdown}</p>
            </div>
          </div>
        )}
        {gameOver && (
